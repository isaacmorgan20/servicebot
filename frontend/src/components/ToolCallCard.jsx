...
const TOOL_CONFIG = {
  lookup_order:    { icon: Search,    label: 'Order Lookup',    color: '#06b6d4' },
  process_refund:  { icon: RotateCcw, label: 'Refund',          color: '#22c55e' },
...
function formatArgs(name, args) {
  if (name === 'lookup_order') return `Order: ${args.order_id || '—'}`
  if (name === 'process_refund') return `Order: ${args.order_id || '—'}`
...
function formatResult(tool) {
  ...
  if (r.customer) return `${r.customer} — ${r.items?.join(', ')} ($${r.total})`
  if (r.refund_amount) return `$${r.refund_amount} refund initiated`
